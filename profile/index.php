...
if ($_SESSION['newEmail'] != null && $_SESSION['newPassword'] != null) {
	...
	if ($valid == false) {
		...
		$user = getUser($_SESSION['newEmail'], $_SESSION['newPassword'], "profile");
	}
} else {
	header('Location: /');
}
...
<div class="middle">
	...
	<div class="middle_right">
		<div class="middle_right_pane">
			...
			<?
			$array = explode(" ", substr($user['balance_history'], 1));
			if ($array[0] == "") {
			...
			foreach ($array as $key) {
				...
				if ($balance[0] == "transfer") {
					?>
					<div>
						<p>Переведено клиенту</p>
						<h3>-<?= $balance[1] ?>₽</h3>
					</div>
					<?
				}
				if ($balance[0] == "addTransfer") {
					?>
					<div>
						<p>Вам переведено</p>
						<h3>+<?= $balance[1] ?>₽</h3>
...
<div class="popout-bg" style="display:none" id="client">
	<div class="popout">
		...
		<form method="POST" action="../imports/actionScript">
			...
			<input type="text" name="client" placeholder="user@example.com" class="input-text" autocomplete="off"
...
<div class="popout-bg" style="display:none" id="settings">
	<div class="popout">
		...
		<form method="POST" action="../imports/actionScript">
			...
			<input type="email" name="data1" placeholder="Новый Email" class="input-text" autocomplete="off"
			       autofocus="off" autosave="off" readonly onfocus="this.removeAttribute('readonly');" id="newEmail">
			<input type="password" name="data2" placeholder="Новый пароль" class="input-text" autocomplete="off"
			       autofocus="off" autosave="off" readonly onfocus="this.removeAttribute('readonly');" id="newPassword">
